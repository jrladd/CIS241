...
<small>Work with a partner to calculate the confidence interval for the mean height in the `dogs` dataframe (remember to use your Pandas cheatsheet and the Guides on our course site):
...
3. Turn the list into a Pandas Series with `pd.Series(your_list)`.
...
- `datetime` is useful for duration, change, and time series plots
- Often converting from a string format to a `datetime` and vice versa
- You can select out specific parts of the date and time
...
pd.to_datetime('2025-05-30')
```

## Text Data

- Data can be a text string but not a category!
- Use the `str` namespace to manipulate strings in Pandas like you would in Python 

```{.python}
# Strings that aren't categories
"Luke Skywalker"
"$50.00"
...
- Latitude and longitude are the most common kind of location data
- Addresses and place names are also location data!
- GIS data often comes in special files and needs special libraries (like Geopandas)

## You Try It!

Work through the examples on different [data types](https://jrladd.com/CIS241/guides/pandas.html#data-types) in the Pandas Guide. Pay special attention to changing a column's data type!
