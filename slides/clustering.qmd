...
1. Run K-Means multiple times in a `for` loop, with a different value for K each time.
...
First, we can look at the relative size of the clusters. Are they relatively balanced? Unbalanced clusters may mean we need to try again. Use `.value_counts()` on the `.labels_` attribute of your model.
...
2. Use the elbow method to determine a good value for K.
